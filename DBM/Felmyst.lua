aura_env.Init = function()
  local FelmystDBM = DBM:GetMod("Felmyst")

  if not FelmystDBM or FelmystDBM.isAugmentedBySolti then
    return
  else
    FelmystDBM.isAugmentedBySolti = true
  end
  ...
  FelmystDBM:AddOption("UseSoltiMod", true, "Use Solti Netherwing boss mod patches")
  FelmystDBM:AddOption("WarnEncapsulate", true, "Warn before Encapsulate cooldown ends")
  FelmystDBM:AddOption("TrackEncapsulate", true, "Show Encapsulate CD")
  ...
  function FelmystDBM:SoltiMod_UnScheduleEnrageTimers()
  ...
  function FelmystDBM:SoltiMod_ScheduleEnrageTimers(delay)
  ...
  function FelmystDBM:SoltiMod_EndAirPhaseStatusBarTimer()
    self:EndStatusBarTimer(ANNOUNCE_DBM.AIR_PHASE, NO_BROADCAST)
  end

  function FelmystDBM:SoltiMod_StartAirPhaseStatusBarTimer(delay)
  ...
  function FelmystDBM:SoltiMod_ScheduleGasNovaAnnounce(delay)
  ...
  function FelmystDBM:SoltiMod_UnScheduleGasNovaAnnounce()
  ...
  function FelmystDBM:SoltiMod_StartGasNovaStatusBarTimer(delay)
    self:StartStatusBarTimer(
      TIMERS[SPELL_IDS.GAS_NOVA] - delay,
  ...
  function FelmystDBM:SoltiMod_EndGasNovaStatusBarTimer()
  ...
  function FelmystDBM:SoltiMod_ScheduleEncapsulateAnnounce(delay)
    self:ScheduleAnnounce(
  ...
  function FelmystDBM:SoltiMod_UnScheduleEncapsulateAnnounce()
    self:UnScheduleAnnounce(ANNOUNCE[SPELL_IDS.ENCAPSULATE].WARN, NO_BROADCAST)
  end

  function FelmystDBM:SoltiMod_StartEncapsulateStatusBarTimer(delay)
    self:StartStatusBarTimer(
      TIMERS[SPELL_IDS.ENCAPSULATE] - delay,
  ...
  function FelmystDBM:SoltiMod_EndEncapsulateStatusBarTimer()
    self:EndStatusBarTimer(ANNOUNCE[SPELL_IDS.ENCAPSULATE].STATUS, NO_BROADCAST)
  end

  function FelmystDBM:SoltiMod_InitGroundPhase(delay)
  ...
  local OnCombatStart = FelmystDBM.OnCombatStart
  function FelmystDBM:OnCombatStart(delay)
  ...
  local OnEvent = FelmystDBM.OnEvent
  function FelmystDBM:OnEvent(event, args)
    OnEvent(self, event, args)
  end

  local OnSync = FelmystDBM.OnSync
  function FelmystDBM:OnSync(msg)
    OnSync(self, msg)
    ...
    if startsWith(msg, SYNC_EVENTS[SPELL_IDS.GAS_NOVA]) then
      self:SoltiMod_EndGasNovaStatusBarTimer()
    ...
    elseif startsWith(msg, SYNC_EVENTS[SPELL_IDS.ENCAPSULATE]) then
      if self.Options.WarnEncapsulate then
    ...
    elseif startsWith(msg, SYNC_EVENTS.AIR_PHASE) and not isAirPhase then
      isAirPhase = true
      self:SoltiMod_UnScheduleGasNovaAnnounce()
      self:SoltiMod_UnScheduleEncapsulateAnnounce()
      self:SoltiMod_EndGasNovaStatusBarTimer()
      self:SoltiMod_EndEncapsulateStatusBarTimer()
    elseif startsWith(msg, SYNC_EVENTS.GROUND_PHASE) then
      isAirPhase = false
